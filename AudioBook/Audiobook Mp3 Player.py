...
root.title("Audiobook MP3 Player")
...
def play_time():
    ...
    song = f'C:/Users/deban/PycharmProjects/pythonProject/AudioBook/mp3Files{song}.mp3'
...
def add_song():
    song = filedialog.askopenfilename(initialdir='mp3Files/', title="Choose A Song",
                                      filetypes=(("mp3 Files", "*.mp3"),))
    # Strip out directory structure and .mp3 from song title
    song = song.replace(r"C:/Users/deban/PycharmProjects/pythonProject/AudioBook/mp3Files", "")
...
def add_many_songs():
    songs = filedialog.askopenfilenames(initialdir='mp3Files/', title="Choose A Song",
                                        filetypes=(("mp3 Files", "*.mp3"),))
...
# Create Function to read PDF File aloud
def read_pdf():
    pdf = filedialog.askopenfilename(initialdir='pdfFiles/', title="Choose A PDF File",
                                     filetypes=(("pdf Files", "*.pdf"),))
    pdfReader = PyPDF2.PdfFileReader(pdf)
    pages = pdfReader.numPages
    sound = ""
    #to loop around pages
    for num in range(0, pages):
        page = pdfReader.getPage(num)
        text = page.extractText() #to extract text from pdf page
        text = text.replace("\n", " ")
        sound = sound + text

    output = gTTS(text=sound, lang='en', slow=True) # using gtts module to get the verbal sound of the text

    # to get the name of the file
    savepath = "C:/Users/deban/PycharmProjects/pythonProject/AudioBook/mp3Files"
    fslash = int(pdf.rindex('/'))
    dot = int(pdf.rindex('.'))
    filename = pdf[fslash + 1:dot] + '.mp3'
    completename = os.path.join(savepath, filename)
    output.save(completename)
    add_many_songs()


# Create Function to read Text File aloud
def read_txt():
    text = filedialog.askopenfilename(initialdir='', title="Choose A Text File", filetypes=(("text Files", "*.txt"),))
    fh = open(text, "r")
    myText = fh.read().replace("\n", " ")
    output = gTTS(text=myText, lang='en', slow=True)
    savepath = "C:/Users/deban/PycharmProjects/pythonProject/AudioBook/mp3Files"
    fslash = int(text.rindex('/'))
    dot = int(text.rindex('.'))
    filename = text[fslash + 1:dot] + '.mp3'
    completename = os.path.join(savepath, filename)
    output.save(completename)
    add_many_songs()
...
# Create Playlist Box
playlist_box = Listbox(main_frame, bg="white", fg="blue", width=60, selectbackground="blue", selectforeground='white')
...
back_btn_img = PhotoImage(file='ButtonImages/back.png')
forward_btn_img = PhotoImage(file='ButtonImages/forward.png')
play_btn_img = PhotoImage(file='ButtonImages/play.png')
pause_btn_img = PhotoImage(file='ButtonImages/pause.png')
stop_btn_img = PhotoImage(file='ButtonImages/stop.png')
...
# Create Delete Song Menu Dropdowns
Add_book_menu = Menu(my_menu, tearoff=0)
my_menu.add_cascade(label="Listen Books", menu=Add_book_menu)
Add_book_menu.add_command(label="Add book in pdf format", command=read_pdf)
Add_book_menu.add_command(label="Add book in text format", command=read_txt)
